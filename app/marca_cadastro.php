<?php include_once('header.php'); include_once('navbar.php'); include_once('db_connect.php'); include_once('utils.php'); is_logged_in()?>
    ...
    <?php
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $exist = pg_query_params($db_connect, 'SELECT * FROM product_brands WHERE name = $1', array($name));
            if (pg_num_rows($exist) > 0) {
                error_msg("Marca ja cadastrada.");
            } else {
                pg_query_params($db_connect, 'INSERT INTO product_brands VALUES(DEFAULT, $1, $2)', array($name, $description));
                sucess_msg("Cadastro concluído!");
            }
        }
    ?>

    <br><br><h1>CADASTRO DE MARCAS</h1>
